...
export const GalleryRow: FC<IGalleryRow> = ({ data,className  }) => {
  ...
  return (
      ...
      <ul className={`${className} wrapper flex items-center justify-center md:justify-start [&_li]:mx-2 md:[&_li]:mx-8 [&_img]:max-w-none`}>
          {data[0].images.map((item: any, idx: number) => (
            <li key={idx} className={`shadow-xl md:w-96 md:h-56 w-48 h-32`}>
              <Image
                alt={"Photo"}
                src={item.url}
                width={384}
